---
title: Agentic Retail with Gemini Enterprise and MCP
...
- **Model Context Protocol (MCP)** for connecting agents dynamically to internal tools and external APIs via standardized MCP servers
...
No prior AI experience is required; most implementation happens through the Google Cloud Console with a small amount of SQL and a single MCP server on Cloud Run.
...
| Cloud Run | Hosting the custom MCP Server containing all tools |
...
Both the conversational shop assistant (Lab 1) and the autonomous engagement agent (Lab 2) require tools to interact with BigQuery and send emails via Gmail. Instead of building separate Cloud Functions and manually writing OpenAPI specifications for each tool, we will deploy a single **Model Context Protocol (MCP) Server** on **Cloud Run**. 

By using MCP, Gemini Enterprise can query our server's endpoint over HTTP via the **StreamableHTTP** transport, automatically discover all exposed tools, and import their parameters and docstrings dynamically.
...
Create a directory `mcp-server/` containing three files: `main.py`, `requirements.txt`, and `Dockerfile`.
...
# Initialize FastMCP
mcp = FastMCP("Retail Agent Tools")
...
def search_products(query: str, top_k: int = 5) -> str:
    """Vector search the product catalog in BigQuery.
    
    Args:
        query: Natural language description of what the customer wants.
        top_k: Max number of products to retrieve.
    """
    ...
    SELECT product_id, name, price_usd, description, features, distance
    FROM VECTOR_SEARCH(
    ...
    for r in results:
        products.append({
            ...
            "features": list(r.features) if r.features else [],
            "distance": float(r.distance)
        })
    return json.dumps(products)

@mcp.tool()
def list_abandoned_orders() -> str:
    """List all orders with status = 'abandoned'."""
    client = bigquery.Client(project=PROJECT)
    sql = "SELECT order_id, customer_id, items, created_at FROM `retail_workshop.orders` WHERE status = 'abandoned'"
    ...
    for r in results:
        orders.append({
            ...
            "items": [{"product_id": item["product_id"], "quantity": item["quantity"]} for item in r.items],
...
def get_customer(customer_id: str) -> str:
    """Get the customer profile by customer_id.
    
    Args:
        customer_id: Customer unique ID.
    """
    client = bigquery.Client(project=PROJECT)
    sql = "SELECT name, email, loyalty_tier, preferred_categories FROM `retail_workshop.customers` WHERE customer_id = @customer_id"
    job_config = bigquery.QueryJobConfig(
    ...
    return json.dumps({
        ...
        "preferred_categories": list(r.preferred_categories) if r.preferred_categories else []
    })

@mcp.tool()
def get_customer_history(customer_id: str) -> str:
    """Retrieve completed orders for a customer.
    
    Args:
        customer_id: Customer unique ID.
    """
    client = bigquery.Client(project=PROJECT)
    sql = "SELECT order_id, items, total_usd, created_at FROM `retail_workshop.orders` WHERE customer_id = @customer_id AND status = 'completed'"
    job_config = bigquery.QueryJobConfig(
        query_parameters=[bigquery.ScalarQueryParameter("customer_id", "STRING", customer_id)]
    )
    results = client.query(sql, job_config=job_config).result()
...
We will create a dedicated Service Account for the MCP Server so that it has exactly the permissions it needs, following the principle of least privilege.

```bash
# Create a service account for the MCP server
gcloud iam service-accounts create mcp-server-sa \
  --display-name="Retail MCP Server Service Account"

# Grant BigQuery read and Secret Manager accessor permissions to the SA
PROJECT=$(gcloud config get-value project)
for ROLE in bigquery.dataViewer bigquery.jobUser secretmanager.secretAccessor; do
  gcloud projects add-iam-policy-binding "$PROJECT" \
...
Save the service URL returned by the deploy command (e.g., `https://retail-mcp-server-xxxxx-uc.a.run.app`). The Agent platform will connect to the `/mcp` endpoint of this URL.
...
### Part 5: Add the MCP server

Gemini Enterprise allows you to define **MCP Server connections** that register all of your server's tools at once. Instead of pasting individual OpenAPI schemas for every function, we will register the deployed Cloud Run URL directly.

1. In the Console, go to **Gemini Enterprise**.

2. Go to the **Tools** tab on the left menu, and click **+ Create tool**.

3. Set:
   - **Tool Type**: `MCP Server`
   - **Display Name**: `Retail MCP Tools`
   - **Server URL**: The HTTPS URL of your deployed Cloud Run service (e.g., `https://retail-mcp-server-xxxxx-uc.a.run.app/mcp`)
   - **Transport**: `StreamableHTTP`
   - **Authentication**: `None` (or `Google IAM` for production, but choose `None` for simplicity in this lab)

4. Click **Save**. Gemini Enterprise will instantly connect to the server, discover the tools (`search_products`, `list_abandoned_orders`, `get_customer`, `get_customer_history`, `send_email`), and import their schemas automatically!

5. Go back to your **Shop Assistant** agent. Under **Playbooks**, click **Tools** and check the box to enable `search_products`.

6. In the Playbook, add an **Example** that shows the agent calling the tool:
...
The agent reads abandoned cart data from BigQuery, cross-references each customer's cart with the catalog, and sends a personalized recovery email through the Gmail API via the MCP server. Differently from a **chatbot**, this **autonomous agent** acts on data.

```mermaid
flowchart LR
  T([Trigger\nrun command]) --> A[Gemini 3.5 Agent]
  A <--> M[MCP Server on Cloud Run]
  M <--> O[(orders\nBigQuery)]
  M <--> C[(customers\nBigQuery)]
  M <--> P[(products\nVECTOR_SEARCH)]
  M <--> G[Gmail API]
...
### Part 3: Enable the MCP tools

Because you connected the MCP Server in Lab 1, all data and email tools are already registered in Gemini Enterprise!

Under the **Tools** section of your duplicated `Post-Purchase Agent`, click **Tools** and check the box to enable all five MCP tools:
...
No additional OpenAPI schemas are required!
...
The agent runs as a Google-managed service account. It needs permission to invoke our authenticated Cloud Run service.
...
# Grant the Gemini agent permission to call the Cloud Run service
gcloud run services add-iam-policy-binding retail-mcp-server \
  --region=us-central1 \
...
The `list_abandoned_orders` tool can be removed entirely. Everything else — Gemini, the `get_customer` / `get_customer_history` / `search_products` tools, and the Gmail `send_email` tool — carries over unchanged.
...
flowchart LR
    ...
    style MS fill:#EA4335,color:#fff
```
